...
def load_agent_settings(config_path: Path) -> dict:
    settings = {
        "step1_model": DEFAULT_GEMINI_MODEL,
        "step1_temp":  0.2,
        "step2_model": DEFAULT_CLAUDE_MODEL,
        "step2_temp":  0.2,
    }
...
def call_gemini_summarize_refs(
    api_key: str,
    references: List[Tuple[str, str]],
    spotchecks: List[Tuple[str, str]],
    model: str,
...
class PathRow(tk.Frame):
    ...
    Each instance keeps its own _initdir so browsing in one row
    never affects the starting directory of any other row.
    """
    ...
    def set(self, value: str) -> None:
        ...
        if value:
            p = Path(value)
            self._initdir = str(p.parent if p.is_file() else p)

    # ── private ───────────────────────────────────────────────────────────────
    def _pick(self):
        if self._mode == "folder":
            selected = filedialog.askdirectory(initialdir=self._initdir)
        else:
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header,
            text="2-Agent Agentic Workflow  |  Gemini + Claude",
        ...
        self.row_articles = PathRow(left, "Article Folder (.docx)", "📄")
        self.row_articles.pack(fill=tk.X, pady=2)

        self.row_refs = PathRow(left, "Reference Materials Folder (.docx)", "📚")
        self.row_refs.pack(fill=tk.X, pady=2)

        self.row_spots = PathRow(left, "Internal Spot Check Folder (.docx)", "🔍")
        self.row_spots.pack(fill=tk.X, pady=2)

        # ── output / config rows ──────────────────────────────────────────────
        section_label(left, "OUTPUT & CONFIGURATION")

        self.row_output = PathRow(left, "Output Folder", "📁")
        self.row_output.pack(fill=tk.X, pady=2)

        self.row_env = PathRow(
            left,
            ".env File  (GOOGLE_API_KEY + ANTHROPIC_API_KEY)", "🔑",
            pick_mode="file",
        ...
        self.log_box.tag_config("agent1",  foreground="#7eb8f7")
    ...
    def _build_agent_cards(self, parent) -> None:
        ...
        for title, model, color, icon, side_pad in [
            ("Agent 1", "Gemini", "#7eb8f7", "🤖", (0, 4)),
            ("Agent 2", "Claude", "#c97bf7", "🧠", (4, 0)),
        ]:
    ...
    def _run_workflow_inner(self) -> None:
        try:
            ...
            if google_key:
                try:
                    self._log("Calling Agent 1 (Gemini) — generating QA rules…")
                    rules_bundle = call_gemini_summarize_refs(
                        google_key, ref_payload, spot_payload,
                        agent_cfg["step1_model"], agent_cfg["step1_temp"],
                    )
                except Exception as e:
                    self._log(f"Gemini failed — using fallback rules  ({e})")
